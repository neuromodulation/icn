...
if __name__ == "__main__":
    ...
    PATH_VHDR = r'C:\Users\ICN_admin\Charité - Universitätsmedizin Berlin\Interventional Cognitive Neuromodulation - Data\Datasets\BIDS_Berlin\sub-002\ses-20200131\ieeg\sub-002_ses-20200131_task-SelfpacedRotationR_acq-MedOn+StimOff_run-4_ieeg.vhdr'
    ...
    gen = ieeg_raw_generator(ieeg_raw[:,:5000], df_M1, settings, fs) # clip for timing reasons 
    

    features_ = features_df.Features(s=settings, fs=fs, line_noise=line_noise, channels=ch_names)

    # call now run_analysis.py 
    df_ = run_analysis.run(gen, settings, df_M1)

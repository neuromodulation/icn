...
def run(gen, settings, df_M1, fs, line_noise, filter_fun, usemean_=True, normalize=True, include_label=False,
        method='bandpass'):
    ...
    while True:
        ...
        if method == 'wavelet1':
            ...
            for idx, f_range in enumerate(settings['frequencyranges']):
                features_sample[:, idx] = np.mean(np.mean(dat_tf[:, (f_range[0] - freq_min)//2:f_range[0]//2, :],
                                                          axis=1)[:, -fs // settings['seglengths'][idx]:], axis=1)
        # feature estimation by wavelet transform
        if method == 'wavelet2':

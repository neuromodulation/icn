...
BIDS_DICTS_Scanlabel = pd.read_excel(
    r'C:\Users\Jonathan\Charité - Universitätsmedizin Berlin\Interventional Cognitive Neuromodulation - Data\BIDS_Tuebingen\Mapping_foldernames_to_BIDSlabels_jvh.xlsx',
    sheet_name='Python')
...
location_sourcedata="C:\\Users\\Jonathan\\Documents\\DATA\\PROJECT_ALSP\\sourcedata"
print("The source folder is:")
print(location_sourcedata)

#location of the output folder
#BIDS_MAIN=location_sourcedata
BIDS_MAIN=r"C:\Users\Jonathan\Documents\DATA\PROJECT_ALSP\rawdata"
...
for Subjectdir in Subjectdirs:
    ...
    # make sure you manually removed all - and changed it to HDLS_
    Subject = "{:03d}".format(int(Subject.group(0)))
    Subject = "sub-" + str(Subject)
    Subject += '-AT' if 'HDLSAT' in Subjectdir else '' #make sure that the HDLS-AT is named HDLSAT in the folder
    #Subject = "sub-001"
    ...
    for i, Sessiondir in enumerate(Sessiondirs):
        ...
        Session = re.search('(?<=u)(([0-9.])*)', Sessiondir) # search for u2021.09.07 or other date
        Session = Session.group(0) # it has to start with u

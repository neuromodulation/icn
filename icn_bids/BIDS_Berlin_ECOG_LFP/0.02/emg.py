...
def raw_plotly(mne_raw, file_name, t_slice=(), plot_title=None,
    ...
    Arguments:
        ...
        do_decimate: downsample (decimate) the signal to 200Hz sampling
            rate (default and recommended value is True)
        do_normalize: dividing the signal by the root mean square value
            for normalization (default and recommended value is True)
    ...
    if t_slice:
        signals_array, time_array = \
            mne_raw[:, t_slice[0]*samp_freq:t_slice[1]*samp_freq]
    else:
...
def sig_plotly(time_array, signals_array, channels_array, samp_freq, file_name,
               plot_title=None, do_decimate=True, do_normalize=True,
               do_detrend="linear"):
    """
    ...
    Arguments:
        ...
        file_name: name (and directory) for the exported html file
        plot_title: Plot title (default is None)
        do_decimate: down-sample (decimate) the signal to 200 Hz sampling rate
            (default and recommended value is True)
        do_normalize: dividing the signal by the root mean square value for
    ...
    if do_detrend == "linear" or do_detrend == "constant":
        signals_array = detrend(signals_array, axis=1, type=do_detrend,
                                overwrite_data=True)
    if do_normalize:
        signals_array = signals_array / rms(signals_array, axis=1).reshape(
            -1, 1)
    offset_value = 4 * rms(signals_array)  # RMS value
    signals_array = signals_array + offset_value * (np.arange(
        len(channels_array)).reshape(-1, 1))
    signals_df = DataFrame(data=signals_array.T, index=time_array,

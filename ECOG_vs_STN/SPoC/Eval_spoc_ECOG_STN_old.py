...
def enet_train(alpha,l1_ratio,x,y):
    reg=ElasticNet(alpha=alpha, l1_ratio=l1_ratio, max_iter=1000)
    #clf.fit(x,y)
    scaler = StandardScaler()
    clf = make_pipeline(scaler, reg)
    cval = cross_val_score(clf, x, y, scoring='r2', cv=3)
...
for m, eeg in enumerate(signal):    
    ...
    for s in range(len(settings['num_patients'])):
        ...
        for ss in range(len(subfolder)):
            ...
            # print('RUNNIN SUBJECT_'+ settings['num_patients'][s]+ '_SESS_'+ str(subfolder[ss]) 'SIGNAL_' + eeg)
    
            list_of_files = os.listdir(settings['out_path']) #list of files in the current directory
            for each_file in list_of_files:
                if eeg=="ECOG":
                    file_name='epochs_10s_sub_' + settings['num_patients'][s] + '_sess_'+subfolder[0][4:]
                else:
                    file_name='STN_epochs_10s_sub_' + settings['num_patients'][s] + '_sess_'+subfolder[0][4:]
            ...
            for l, mov in enumerate(laterality):
                ...
                for train_index, test_index in cv.split(aux):
                    ...
                    # clf=ElasticNet(alpha=optimizer.x[0], l1_ratio=optimizer.x[1], max_iter=1000, normalize=False)
            ...
            out_path_file = os.path.join(settings['out_path_process']+ settings['num_patients'][s]+'predictions_'+eeg+'_tlag_10s_'+str(subfolder[ss])+'.npy')
        ...
        ind_best=np.argmax(score_tr['CON'])
        ...
        fig.suptitle('ECOG-Subject_'+ settings['num_patients'][s], fontsize=14, fontweight='bold')
        plt.legend()
        plt.show()

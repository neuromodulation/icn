...
settings['frequencyranges']=[60, 200]
        ...
        for ss in range(len(subfolder)):
            ...
            for l, mov in enumerate(laterality):
                ...
                for train_index, test_index in cv.split(label):
                    ...
                    #apply ssd as a denoising method
                    Xtr=ssd.fit_transform(Xtr)
                    Xte=ssd.transform(Xte)
                                 
                    #apply spoc on denoised data
                    ...
                    gtr=gtr.var(axis=1)
                    #normalize for r2 scoring comparisons
            ...
            out_path_file = os.path.join(settings['out_path_process']+ settings['num_patients'][s]+'predictions_'+eeg+'_space_ssd'+'_'+str(subfolder[ss])+'.npy')

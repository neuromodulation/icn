...
PATH_SAVE = 'C:\\Users\\ICN_admin\\Documents\\ECOGVSTN\\ICN_previous\\SUB_005\\005_CONCAT\\' # WHERE TO SAVE THE CONCATENATED FILE
PATH_OUT = 'C:\\Users\\ICN_admin\\Documents\\ECOGVSTN\\ICN_previous\\SUB_005\\' # PATH WHERE THE RAW RAW_SUB_RUN_SESS.json files are saved
...
def write_patient_concat_ch(subject_id, BIDS_path=BIDS_PATH, path_out=PATH_SAVE, preprocessed_path=PATH_OUT):
...
def write_all_rawcombined():
    ...
    """
    subject_id = []
    for patient_idx in np.arange(16):
        if patient_idx < 10:
...
def run_CV_est(subject_id, model_=linear_model.LinearRegression(),out_path = PATH_OUT,
        LM_=True, classification=False, path_data=PATH_SAVE, time_stamps=5, time_shift=0):
...
if __name__ == "__main__":
    ...
    #write_patient_concat_ch('005')

    #run_CV_est(subject_id='005', model_=linear_model.LinearRegression(),out_path = PATH_OUT,
    #    LM_=True, classification=False, path_data=PATH_SAVE, time_stamps=5, time_shift=0)
    ...
    pool.map(partial(run_CV_est, model_=model, out_path=out_here, LM_=True, path_data=PATH_OUT,
        time_stamps=1, time_shift=0, classification=False), subject_id)
    ...
    if eval_differemt_models is True:
        ...
        for time_shift in range(11):
            ...
            pool = multiprocessing.Pool(processes=62)
            pool.map(partial(run_CV_est, model_=model, out_path=out_here, LM_=True, path_data=PATH_DOWNSAMPLED,
        ...
        if use_LM is True:
            ...
            for t in np.arange(100, 1100, 100):
                ...
                model = linear_model.LinearRegression()
                time_idx = time_idx + 1
                pool.map(partial(run_CV_est, model_=model, out_path=out_here, LM_=True, path_data=PATH_DOWNSAMPLED, time_stamps=time_idx), subject_id)

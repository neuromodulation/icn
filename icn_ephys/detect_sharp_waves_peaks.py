...
<<<<<<< HEAD
        ...
        peak_dist=5; trough_dist=1; label=True; y_contra=None; y_ipsi=None; plot_=False
=======
        peak_dist=5;
        trough_dist=1;
        label=True;
...
>>>>>>> fbdcc8087a7fab3f01f708bd7f0a0ab72512ecbc
        ...
        for peak_idx in peaks:
            try:
                trough_idx_left, trough_idx_right, trough_left, trough_right = self.get_peaks_around(peak_idx,
                                                                                        troughs, filtered_dat)
            ...
            # interval
            if df.shape[0]>0:
                interval_ = (peak_idx - sharp_wave["peak_idx"]) * (1000/self.sample_rate)
            else:
                # set first interval to zero
                interval_ = 0

            # sharpness
            if (peak_idx - int(5*(1000/self.sample_rate)) <= 0) or \
                (peak_idx + int(5*(1000/self.sample_rate)) >= filtered_dat.shape[0]):
                continue
            # convert 5 ms to sample rate
            sharpness = ((filtered_dat[peak_idx] - filtered_dat[peak_idx-int(5*(1000/self.sample_rate))]) +
                         (filtered_dat[peak_idx] - filtered_dat[peak_idx+int(5*(1000/self.sample_rate))])) / 2

            # rise_steepness, first der. from trough to peak
            # here  + 1 due to python syntax, s.t. the last element is included
            rise_steepness = np.max(np.diff(filtered_dat[trough_idx_left : peak_idx+1]))

            # decay_steepness
            decay_steepness = np.max(np.diff(filtered_dat[peak_idx : trough_idx_right+1]))

            sharp_wave  = {
                "trough_left" : trough_left,
                "trough_right" : trough_right,
                "trough_idx_left" : trough_idx_left,
                "trough_idx_right" : trough_idx_right,
                "peak" : filtered_dat[peak_idx], # mV
                "peak_idx" : peak_idx,
                "width" : trough_idx_right - trough_idx_left, # ms
                "prominence": np.abs(filtered_dat[peak_idx] - (trough_right + trough_left) / 2), # mV
                "interval" : interval_, # ms
                "decay_time": (trough_idx_right - peak_idx) * (1000/self.sample_rate),
                "rise_time" : (peak_idx - trough_idx_left) * (1000/self.sample_rate),
                "sharpness" : sharpness,
            ...
            if label is True:
                sharp_wave["label"] = True
                    # movement
                if y_ipsi[peak_idx] > 0:
                    MOV_ = "IPS"
                elif y_contra[peak_idx] > 0:
                    MOV_ = "CON"
                else:
                    MOV_ = "NO_MOV"
                sharp_wave["MOV_TYPE"] = MOV_
                sharp_wave["y_contra"] = y_contra[peak_idx]
                sharp_wave["y_ipsi"] = y_ipsi[peak_idx]
